...
var cssPlugins = [cssAutoprefixer(),
                  cssmodules({
...
gulp.task('dev:reload', function() {
    gulp.src([srcAll, '!' + srcLess])
        .pipe(watch([srcAll, '!' + srcLess]))
        .pipe(connect.reload());
...
gulp.task('dist:image', ['dist:copy'], function(cb) {
    ...
    var targetGif = path.join(targetDir, '/**/*.gif');

    var targetDistDir = path.join(distDir, targetDir.slice(srcDir.length));

    gulp.src([targetPng, targetJpg, targetGif])
        .pipe(minifyImg({
            progressive: true,
